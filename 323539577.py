...
def learn_language_model(files, n=3, lm=None):
    ...
    ngrams_list = ngrams(words, n, True, True, '', '')
...
def create_error_distribution(errors_files, lexicon):
    ...
    for line in flat_list:
        ...
        if len(err) < len(corr):
            count = distribution_dict[''.join(diff_del(err, corr))[::-1]] if distribution_dict[
                                                                                 ''.join(diff_del(err, corr))[
                                                                                 ::-1]] > 0 else total_count
            error_dict['deletion'][diff_del(err, corr)] += 1 / count
        elif len(err) > len(corr):
            count = distribution_dict[diff_insertion(err, corr)[1]] if distribution_dict[diff_insertion(err, corr)[
                1]] > 0 else total_count
            error_dict['insertion'][diff_insertion(corr, err)] += 1 / count
        else:
            if set(err) != set(corr):
                count = distribution_dict[diff_sub(err, corr)[1]] if distribution_dict[
                                                                         diff_sub(err, corr)[1]] > 0 else total_count
                ...
                for k_err, v_err in err_counter.items():
                    if v_err != corr_counter[k_err]:
                        count = distribution_dict[diff_sub(err, corr)[1]] if distribution_dict[diff_sub(err, corr)[
                            1]] > 0 else total_count
                        error_dict['substitution'][diff_sub(err, corr)] += 1 / count
                        flag = False
                        break
                if flag:
                    count = distribution_dict[diff_trans(err, corr)[1]] if distribution_dict[diff_trans(err, corr)[
                        1]] > 0 else total_count
...
def get_probability(err_dist, error, func, err, corr, total_count=None, word_counts=None):
    if not total_count and not word_counts:
        return 1 if err_dist[error][func(err, corr)] == 0 else err_dist[error][func(err, corr)]

    channel_model = 1 if err_dist[error][func(err, corr)] == 0 else err_dist[error][func(err, corr)]
...
def correct_word(w, word_counts, err_dist, c=1):
    ...
    for corr in word_small_distnace:
        if len(err) < len(corr):
            probability_dict[corr] = get_probability(err_dist, 'deletion', diff_del, err, corr, total_count,
                                                     word_counts)
        elif len(err) > len(corr):
            probability_dict[corr] = get_probability(err_dist, 'insertion', diff_insertion, err, corr, total_count,
                                                     word_counts)
        else:
            if set(err) != set(corr):
                ...
                for k_err, v_err in err_counter.items():
                    if v_err != corr_counter[k_err]:
                        ...
                        break
                if flag:
                    probability_dict[corr] = get_probability(err_dist, 'transposition', diff_trans, err, corr,
                                                             total_count,
                                                             word_counts)
...
def correct_sentence(s, lm, err_dist, c=1, alpha=0.95):
    ...
    n = extract_n(lm)
    ngrams_list = ngrams(s, n, True, True, '', '')
    ngram_dict = {}
    for temp_ngram in ngrams_list:
        ngram_dict[temp_ngram[-1]] = temp_ngram[:-1]
    lexicon = get_lex(lm)
    ...
    for err, corrs in possible_words.items():
        for corr in corrs:
            ...
            lm_prob = evaluate_text(' '.join(s).replace(err, corr), n, new_lm, new_ngram_dict)
            #print('Possible Sent: ' +' '.join(s).replace(err, corr)+'. Noisy channel prob (word): ' +str(noisy_channel)+' Prior/LM prob (probability for sentence with the possible word): '+str (lm_prob)+' Final prob fot the word '+ str( math.exp(math.log(noisy_channel) +math.log(lm_prob))))
    ...
    s = ' '.join(s).replace(max_tuple[0], max_tuple[1])
    return s
...
def get_lex(lm):
    lex = set()
    for ngram in lm:
        lex.update([normalization(x) for x in ngram])
    return lex
